...
function fillSettingsForm() {
  ...
  const tok = document.getElementById('settingsToken');
  const pth = document.getElementById('settingsPath');
  const srv = document.getElementById('settingsServer');
  if (tok && s.token)  tok.value = s.token;
  if (pth && s.path)   pth.value = s.path;
  ...
  Object.entries(fm).forEach(([id, key]) => {
    const el = document.getElementById(id);
    if (el && DEFAULT_FIELDS && DEFAULT_FIELDS[key]) {
